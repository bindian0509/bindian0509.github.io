const socialLinks = [
  {
    name: "LinkedIn",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
    href: "https://linkedin.com/in/bharatverma",
    color: "#0A66C2",
  },
  {
    name: "Twitter",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
      </svg>
    ),
    href: "https://twitter.com/iambharatv",
    color: "#1DA1F2",
  },
  {
    name: "GitHub",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.435 9.795 8.205 11.385.6.105.825-.255.825-.57 0-.285-.015-1.23-.015-2.235-3.015.555-3.795-.735-4.035-1.41-.135-.345-.72-1.41-1.23-1.695-.42-.225-1.02-.78-.015-.795.945-.015 1.62.87 1.845 1.23 1.08 1.815 2.805 1.305 3.495.99.105-.78.42-1.305.765-1.605-2.67-.3-5.46-1.335-5.46-5.925 0-1.305.465-2.385 1.23-3.225-.12-.3-.54-1.53.12-3.18 0 0 1.005-.315 3.3 1.23.96-.27 1.98-.405 3-.405s2.04.135 3 .405c2.295-1.56 3.3-1.23 3.3-1.23.66 1.65.24 2.88.12 3.18.765.84 1.23 1.905 1.23 3.225 0 4.605-2.805 5.625-5.475 5.925.435.375.81 1.095.81 2.22 0 1.605-.015 2.895-.015 3.3 0 .315.225.69.825.57A12.02 12.02 0 0024 12c0-6.63-5.37-12-12-12z" />
      </svg>
    ),
    href: "https://github.com/bindian0509",
    color: "#f0f6fc",
  },
  {
    name: "Medium",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M13.54 12a6.8 6.8 0 01-6.77 6.82A6.8 6.8 0 010 12a6.8 6.8 0 016.77-6.82A6.8 6.8 0 0113.54 12zM20.96 12c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75-.66 0-1.19-2.58-1.19-5.75s.53-5.75 1.19-5.75C23.47 6.25 24 8.83 24 12z" />
      </svg>
    ),
    href: "https://bharatv90s.medium.com",
    color: "#f0f6fc",
  },
  {
    name: "Dev.to",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M7.42 10.05c-.18-.16-.46-.23-.84-.23H6v4.36h.58c.37 0 .65-.08.84-.23.21-.16.31-.44.31-.83v-2.24c0-.39-.1-.67-.31-.83zm7.27-1.75c-.27 0-.52.09-.72.29s-.3.43-.3.73v4.95c0 .3.1.54.3.74s.44.29.71.29c.27 0 .52-.09.72-.29s.3-.43.3-.73V9.32c0-.3-.1-.54-.3-.74s-.44-.29-.71-.29zM19.59 3H4.41C3.08 3 2 4.08 2 5.41v13.18C2 19.92 3.08 21 4.41 21h15.18c1.33 0 2.41-1.08 2.41-2.41V5.41C22 4.08 20.92 3 19.59 3zM8.81 14.05c0 .92-.52 1.73-1.64 1.73H5.16V8.22h2.01c1.12 0 1.64.81 1.64 1.73v4.1zm4.86-1.38c0 1.16-.57 2.19-1.81 2.19-1.24 0-1.81-1.03-1.81-2.19V9.55c0-1.16.57-2.19 1.81-2.19 1.24 0 1.81 1.03 1.81 2.19v3.12zm5.17-2.03h-1.63v1.24h1.28v1.29h-1.28v1.64h1.63v1.29h-2.91V8.22h2.91v1.42z" />
      </svg>
    ),
    href: "https://dev.to/bindian0509",
    color: "#f0f6fc",
  },
  {
    name: "Telegram",
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
        <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
      </svg>
    ),
    href: "https://t.me/BIRD_PERSON_27",
    color: "#26A5E4",
  },
];

export default function Contact() {
  return (
    <section id="contact" className="py-20">
      <div className="max-w-4xl mx-auto px-6">
        {/* Section Header */}
        <div className="text-center mb-16">
          <p className="text-[#58a6ff] font-mono text-sm mb-2">05. Contact</p>
          <h2 className="text-3xl md:text-4xl font-bold text-[#f0f6fc] mb-4">
            Let&apos;s Connect
          </h2>
          <p className="text-[#8b949e] text-lg max-w-2xl mx-auto">
            Whether you want to discuss engineering leadership, system design, or just want to say hi,
            my inbox is always open. Feel free to reach out!
          </p>
        </div>

        {/* Email CTA */}
        <div className="text-center mb-12">
          <a
            href="mailto:bharatv@outlook.in?subject=Hello%20from%20your%20portfolio&body=Hi%20Bharat,"
            className="inline-flex items-center gap-3 px-8 py-4 bg-[#1f6feb] hover:bg-[#58a6ff] text-white rounded-lg font-medium text-lg transition-all hover:scale-105"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            Say Hello
          </a>
        </div>

        {/* Social Links */}
        <div className="flex flex-wrap justify-center gap-4">
          {socialLinks.map((link, index) => (
            <a
              key={index}
              href={link.href}
              target="_blank"
              rel="noopener noreferrer"
              className="group flex items-center gap-3 px-5 py-3 bg-[#21262d] border border-[#30363d] rounded-lg hover:border-[#58a6ff] transition-all card-hover"
            >
              <span
                className="transition-colors"
                style={{ color: link.color }}
              >
                {link.icon}
              </span>
              <span className="text-[#8b949e] group-hover:text-[#f0f6fc] transition-colors">
                {link.name}
              </span>
            </a>
          ))}
        </div>

        {/* Ask Me About */}
        <div className="mt-16 bg-[#161b22] border border-[#30363d] rounded-lg p-8">
          <h3 className="text-xl font-semibold text-[#f0f6fc] mb-6 text-center">
            💬 Ask Me About
          </h3>
          <div className="grid md:grid-cols-2 gap-4">
            {[
              { topic: "System Design", desc: "Distributed systems, microservices, high-scale architectures" },
              { topic: "Payment Systems", desc: "Card payments, recurring transactions, PCI compliance" },
              { topic: "Engineering Leadership", desc: "Team building, hiring, performance management" },
              { topic: "Tech Stack", desc: "Java, Go, Spring Boot, Kubernetes, AWS, Kafka" },
              { topic: "DevOps", desc: "CI/CD pipelines, infrastructure as code, observability" },
              { topic: "Interview Prep", desc: "System design interviews, coding rounds, leadership rounds" },
            ].map((item, index) => (
              <div key={index} className="flex items-start gap-3">
                <span className="text-[#3fb950] mt-1">▹</span>
                <div>
                  <span className="text-[#f0f6fc] font-medium">{item.topic}</span>
                  <span className="text-[#8b949e]"> → {item.desc}</span>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}

